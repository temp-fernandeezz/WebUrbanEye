<footer class="bg-[#00261a]">
    <div class="mx-auto max-w-7xl p-6">
        {{-- <div class="flex gap-8 justify-center items-center">
            <div>
                <a href="{{ route('about') }}">
                    <p class="text-md text-[#E3B873]"> Sobre nós</p>
                </a>
            </div>
            <div>
                <a href="{{ route('location') }}">
                    <p class="text-md text-[#E3B873]">Buscar locais</p>
                </a>
            </div>
            <div>
                <a href="{{ route('dashboard') }}">
                    <p class="text-md text-[#E3B873]">Realizar Reclamações</p>
                </a>
            </div>
            <div>
                <a href="{{ route('blog') }}">
                    <p class="text-md text-[#E3B873]">Blog</p>
                </a>
            </div>
            <div>
                <a href="{{ route('login') }}">
                    <p class="text-md text-[#E3B873]">Login</p>
                </a>
            </div>
            <div>
                <a href="{{ route('register') }}">
                    <p class="text-md text-[#E3B873]">Cadastre-se</p>
                </a>
            </div>
        </div> --}}

        <div>
            <p class="text-white text-center pt-4 text-sm">© Todos os direitos reservados. UrbanEye. </p>
        </div>
    </div>
</footer>
